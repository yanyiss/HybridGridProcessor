...
namespace CADMesher
{
	void OccReader::tempTest()
	{
#if 1
		std::vector<std::vector<double>> cdpara(7);
		cdpara[0]={
		0, 0,
		0.1, 0,
		0.15, 0,
...
#endif
		Matrix2Xd all_pnts(2, 267);
		int s = 0;
		int begin = 0;
		int end = 17;
		for (; begin < end; ++begin, ++s)
		{
			all_pnts.col(begin) << cdpara[0][2 * s], cdpara[0][2 * s + 1];
		}
		end += 18;
		s = 0;
		for (; begin < end; ++begin, ++s)
		{
			all_pnts.col(begin) << cdpara[1][2 * s], cdpara[1][2 * s + 1];
		}
		end += 18;
		s = 0;
		for (; begin < end; ++begin, ++s)
		{
			all_pnts.col(begin) << cdpara[2][2 * s], cdpara[2][2 * s + 1];
		}
		end += 18;
		s = 0;
		for (; begin < end; ++begin, ++s)
		{
			all_pnts.col(begin) << cdpara[3][2 * s], cdpara[3][2 * s + 1];
		}
		end += 97;
		s = 0;
		for (; begin < end; ++begin, ++s)
		{
			all_pnts.col(begin) << cdpara[4][2 * s], cdpara[4][2 * s + 1];
		}
		end += 97;
		s = 0;
		for (; begin < end; ++begin, ++s)
		{
			all_pnts.col(begin) << cdpara[5][2 * s], cdpara[5][2 * s + 1];
		}
		end += 2;
		s = 0;
		for (; begin < end; ++begin, ++s)
		{
			all_pnts.col(begin) << cdpara[6][2 * s], cdpara[6][2 * s + 1];
		}

		vector<Matrix2Xi> bnd(2);
		bnd[0].resize(2, 71);
		bnd[1].resize(2, 196);
		for (int i = 0; i < 70; ++i)
		{
		...
		{
			bnd[1].col(i - 71) << i, i + 1;
		}
		bnd[1].col(195) << 266, 71;
		Surface_TriMeshes.resize(1);
		triangulate(all_pnts, bnd, -1.0, Surface_TriMeshes[0]);
	...
	void OccReader::Discrete_Edge()
	{
		...
		for(auto edge = edgeshape.begin(); edge != edgeshape.end(); ++edge)
		{
			...
			int segment_number = std::max(1/*BRep_Tool::IsClosed(aedge) ? 4 : 4*/, int(curve_length / expected_edge_length));
