...
#pragma omp parallel for
	...
	*/
				...
				//file_writer << "uv\n";
				//file_writer << u.size() << " " << v.size() << std::endl;
				//for (auto uu : u)
				//	file_writer << uu << " ";
...
#pragma omp parallel for
				for (int jj = 0; jj < vertex_surface_index[ii].size(); ++jj)
				{
					...
					if (mesh->data(tv).get_vertflag())
						continue;
					OpenMesh::Vec3d pos = mesh->point(tv);
					//file_writer << pos[0] << " " << pos[1] << " " << pos[2] << std::endl;
					//continue;
	...
	*/
	...
	void MeshProjectToSurface(PolyMesh* mesh, vector<vector<unsigned>> &vertex_surface_index) {
		vector<ShapeFace> &faceshape = globalmodel.faceshape;
		//vector<vector<unsigned>>::iterator index_itr = vertex_surface_index.begin();
		vector<int> surface_type(10, 0);
...
//#pragma omp parallel for
	...
	*/
					...
					for (auto ve : mesh->ve_range(tv))
						avgl = std::min(avgl, (double)mesh->calc_edge_length(ve));
					//dprint(jj, (openp - pos).norm() / mesh->valence(tv), avgl);
					if ((openp - pos).sqrnorm() * 1000000 * mesh->valence(tv)*mesh->valence(tv) > avgl*avgl)
						continue;
					mesh->set_point(tv, openp);
					proj_count[id] = true;
				}
