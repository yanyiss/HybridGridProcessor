...
void TriMeshQualityHelper::update()
{
	double c = 4 * std::sqrt(3);
	minAngle = 4; maxAngle = 0; avgAngle = 0;
	minQuality = 1; avgQuality = 0;
	...
	avgAngle /= 3 * mesh->n_faces();
	avgQuality /= mesh->n_faces();
}

void TriMeshQualityHelper::print()
{
	dprint("min, max, avg angle:", minAngle, maxAngle, avgAngle);
...
void polycopy(Mesh &tm, Mesh &m, bool update_property)
{
	m.clear();
	for (auto tv = tm.vertices_begin(); tv != tm.vertices_end(); ++tv)
	{
