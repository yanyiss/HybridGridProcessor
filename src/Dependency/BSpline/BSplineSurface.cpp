...
BSplineSurface::BSplineSurface(int udeg, int vdeg, std::vector<double> &uknots, std::vector<double> &vknots, std::vector<std::vector<double>> &w, std::vector<std::vector<Point>> &controlpoints)
{
	...
	UV = Point4(uknots[u_degree], uknots[GetNumOfCtrlptsU()], vknots[v_degree], vknots[GetNumOfCtrlptsV()]); //2022.03.21
}
...
bool BSplineSurface::PrincipalCurvature(const double u, const double v, double &k1, double &k2) const  //计算主曲率
{
